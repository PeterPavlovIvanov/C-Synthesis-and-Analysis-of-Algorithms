...
BOOL ShelvesDialog::DialogToBuf()
{
	...
	//shelf's rack
	long lIndex = comboShelfsRack.GetCurSel();
	if (lIndex == CB_ERR)
	{
		message.AppendFormat(_T("\nA rack must be selected."));
		bResult = FALSE;
	}
	openShelf.rackID = (long)comboShelfsRack.GetItemData(lIndex);
...
void ShelvesDialog::FillComboBoxRacks()
{
	...
	{
		int nIndex = comboShelfsRack.AddString(racksArray.GetAt(i)->rackName);
		comboShelfsRack.SetItemData(nIndex, racksArray.GetAt(i)->ID);
	}
	...
	if (dialogMode != DialogModes::DialogModeInsert)
	{
		...
		for (int i = 0; i < racksArray.GetCount(); i++)
			if (openShelf.rackID == racksArray.GetAt(i)->ID)
			{
				...
				int lIndex = comboShelfsRack.SelectString(0, stringRackName);
				comboShelfsRack.SetCurSel(lIndex);
